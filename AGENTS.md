...
2. `director-only`：完整文稿或实录到全文语义四路导演、真实素材执行单、AI/纸艺首帧与视频提示词、拍后 Shotcraft 机会，并建立本期唯一素材投放入口。
...
- 唯一内容交付是：`完整文稿或实录 → 全文语义 beat → 真人/真实素材/AI情景/纸艺四路择一 → 三条素材执行包 → 连续真人节奏复核 → 拍后 Shotcraft 机会`，状态最高只能是 `ready-for-user-review`。
- 导演方案交付时必须同步完成本地素材入口：以本期项目目录为唯一真实目录，创建 `01_素材投放区/` 及口播原片、用户录屏与演示、AI 情景视频、纸艺视频、Codex 公开素材、其他授权素材六个分区；再在 `~/Desktop/口播素材/` 建立一个指向该本期目录的单一符号链接。桌面入口只方便用户投放，不得作为生产合同、哈希或渲染的规范路径。
- 交付前必须核对桌面入口实际解析到本期唯一项目目录，且投放说明存在；在本期控制目录写入 `桌面入口回执.v1.json`，记录入口、目标、实际解析路径和未创建重复工程。若同名桌面项已存在但不是正确链接，禁止覆盖，状态写为 `素材入口待处理`；不得声称本期导演交接完整。建立空目录和说明不等于搜索、生成、下载、剪辑或获得下游授权。
