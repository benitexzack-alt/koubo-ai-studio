...
def source_record(path: Path) -> dict[str, str]:
    ...
    if completeness not in SOURCE_ALLOWED:
        raise ValueError(f"来源不是完整可分析材料：{path}（source_completeness={completeness}）")
    rights_match = SOURCE_RIGHTS.search(text[:8192])
    return {
        "id": f"source:{sha256(path)[:16]}",
        "path": str(path),
        "sha256": sha256(path),
...
def main() -> int:
    ...
    try:
        ...
        sources = [source_record(source.expanduser().resolve()) for source in args.source]
