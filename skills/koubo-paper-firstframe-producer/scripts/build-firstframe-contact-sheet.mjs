...
try {
  ...
  scenes.forEach((scene) => {
    if (!scene?.result || !existsSync(scene.result.imagePath)) throw new Error(`IMAGE_RESULT_MISSING:${scene?.sceneId}`);
  });
  const columns = Math.ceil(Math.sqrt(scenes.length));
  const rows = Math.ceil(scenes.length / columns);
  const cellWidth = 640;
  const cellHeight = 360;
  const inputArgs = scenes.flatMap((scene) => ['-i', scene.result.imagePath]);
  ...
  const outputPath = path.join(job.output.qaRoot, `${args.phase}-contact-sheet.jpg`);
  ...
  console.log(JSON.stringify({ok: true, outputPath, sceneIds, width: columns * cellWidth, height: rows * cellHeight}));
} catch (error) {
