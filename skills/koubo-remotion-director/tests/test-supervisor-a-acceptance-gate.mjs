...
const canonicalPackageRoot = path.join(
  repoRoot,
  'work/director-paper-editorial/20260824-wechat-real-input/director-skill-recovery/dynamic-gate-01/a-progressive-states-canonical',
);
const legacyRequestPath = path.join(
  repoRoot,
  'work/director-paper-editorial/20260824-wechat-real-input/director-skill-recovery/dynamic-gate-01/director-request.exact30.json',
);
...
function buildAcceptedCompletionContext() {
  ...
  const acceptancePath = path.resolve(repoRoot, ...anchor.receipt.path.split('/'));
  const machineReviewPath = path.resolve(repoRoot, ...anchor.machineReview.path.split('/'));
  const visualReviewPath = path.resolve(repoRoot, ...anchor.visualReview.path.split('/'));
