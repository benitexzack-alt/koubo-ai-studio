...
同一镜同时使用毫米机构和确定性布局参考时，须在`motionContract.projectionContract`绑定统一投影，从完整活动件包络推导`sweptRect`并联验标签、字幕、内容区、支承与跨组通道障碍；不能分别手填两套坐标后声称相容。投影参数改变后必须重算动作区、重新独立复核并新建导演revision。未配置投影的历史镜头仅保持原检查，不得冒称通过跨坐标联验；参考及实际生成图仍各自验收，计算通过不代表模型服从。
