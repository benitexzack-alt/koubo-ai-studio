...
if (usesGeneratedBackground && (!task.aiDisclosure?.required || !task.aiDisclosure?.label)) {
  errors.push('使用 AI 背景时必须记录 aiDisclosure.required=true 和标识文案。');
}

if (args.includes('--ocr') && statusIndex >= statuses.indexOf('grid-ready')) {
  for (const candidate of candidates) {
    ...
    const hits = expectedLines.filter((line) => actual.includes(line)).length;
