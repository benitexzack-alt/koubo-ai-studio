...
// 本轮仅处理已获用户批准的 P01，原图与导演包均保持只读。
const base = path.resolve(import.meta.dirname, '..');
const source = path.join(base, 'paper-v9.1-r4/first-frame-production-r1/first-frames/P01_B04_first-frame.png');
const outputRoot = path.join(base, 'paper-v9.1-r4/first-frame-local-reframe-P01-r1');
const output = path.join(outputRoot, 'first-frames/P01_B04_first-frame-reframed.png');
const receiptPath = path.join(outputRoot, 'local-reframe-receipt.v1.json');
const expectedSha = 'db8bfd45cf1393ccfab570cd7268e375c6143acd4a4a222696aefdd5ee338f50';
...
const sourceBounds = [87, 157, 1581, 741];
...
const receipt = {
  ...
  createdAt: new Date().toISOString(), sceneId: 'P01',
  status: 'geometry-transform-checked-awaiting-actual-visual-review',
  authority: {userQuote: '好的，那就抓紧来处理。', acceptedProposal: '先用本地工具将P01整组等比缩小并补背景，不重绘、不裁掉底台和支脚，再检查质量。'},
