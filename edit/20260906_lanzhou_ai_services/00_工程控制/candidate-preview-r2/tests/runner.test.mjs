...
import {ROOT, KB, CONTROL, EPISODE, REVISION, ENTRY, PUBLIC, PERMISSION, OUTPUT, RENDER, COMPOSITION, BINARIES,
  ...
  validateRequest, validatePermission, validatePreservedInputs, assertSnapshot, collectLocalImports, cleanEnvironment, sandboxProfile, loadTypescript} from '../runner-core.mjs';
import {parseCli, validateMediaProbe, validatePng, postRenderProbeOptions} from '../runner.mjs';
...
function fixture() {
  ...
  const dataPath = `edit/${EPISODE}/04_导演拆解/candidate-preview-r2/data.v1.json`;
  ...
  const extraPublic = ['R01.mp4', 'U01.mp4', 'captions.json', 'O01-title.png', 'O01-onsite.png', 'sfx/card.wav', 'sfx/tick.wav'];
  for (const name of extraPublic) put(path.join(root, PUBLIC, name), 'non-media-public-fixture');
  ...
  const paths = [ENTRY, dataPath, `${PUBLIC}/fonts/STHeiti-Medium.ttc`, ...permission.acceptedPaperExceptions.map(s => `${PUBLIC}/${s.sceneId}.mp4`), ...extraPublic.map(n => `${PUBLIC}/${n}`)];
  permission.preservedInputs = [{path: dataPath, sha256: hashFile(path.join(root, dataPath))}];
  permission.preservedPublicFiles = paths.filter(n => n.startsWith(`${PUBLIC}/`)).map(n => ({path: n, sha256: hashFile(path.join(root, n))}));
  permission.approvedAdditionalPublicSfx = [];
...
test('R2 refuses R1 authorization and uses its own public directory', () => {
  const f = fixture();
  const r1 = readJson(path.join(ROOT, `edit/${EPISODE}/00_工程控制/candidate-preview-r1/permission.v1.json`));
  assert.throws(() => validatePermission(r1, f.request, 'render'), /OLD_GRANT/);
  assert.equal(PUBLIC, 'remotion/public/lanzhou-services-candidate-r2');
  assert.equal(OUTPUT.endsWith('/candidate-preview-r2'), true);
  assert.equal(COMPOSITION, 'LanzhouServicesV91CandidateR2');
});

test('R2 admits the requested 55 stills and rejects more than 64', () => {
  const f = fixture(); f.request.stillFrames = Array.from({length: 55}, (_, i) => i); f.seal();
  assert.equal(f.validate('stills').request.stillFrames.length, 55);
  f.request.stillFrames = Array.from({length: 65}, (_, i) => i); f.seal();
  assert.throws(() => f.validate('stills'), /STILL_RANGE_INVALID/);
});

test('R2 preserves timebase and requires confirmation for each new public SFX', () => {
  const f = fixture(); const refs = new Map(f.request.bindings.map(b => [b.path, b.sha256]));
  const names = f.permission.preservedPublicFiles.map(b => b.path);
  assert.doesNotThrow(() => validatePreservedInputs(f.permission, refs, names));
  refs.set(f.dataPath, digest('changed timing'));
  assert.throws(() => validatePreservedInputs(f.permission, refs, names), /PRESERVED_INPUT_DRIFT/);
  refs.set(f.dataPath, f.permission.preservedInputs[0].sha256);
  const extra = {path: `${PUBLIC}/sfx/new.wav`, sha256: digest('new local sound')};
  refs.set(extra.path, extra.sha256); names.push(extra.path);
  assert.throws(() => validatePreservedInputs(f.permission, refs, names), /UNAPPROVED_PUBLIC_CHANGE/);
  f.permission.approvedAdditionalPublicSfx.push(extra);
  assert.throws(() => validatePreservedInputs(f.permission, refs, names), /NEW_SFX_CONFIRMATION_REQUIRED/);
  extra.confirmationQuote = 'fixture-only explicit parent confirmation';
  assert.doesNotThrow(() => validatePreservedInputs(f.permission, refs, names));
});
...
for (const [name, mutation, code] of [
  ...
  ['missing preserved import', f => f.request.bindings = f.request.bindings.filter(b => b.path !== f.dataPath), /PRESERVED_INPUT_DRIFT/],
...
test('post-render bundled ffprobe uses BINARIES cwd under unchanged sandbox', {skip: process.platform !== 'darwin'}, () => {
  const scratch = path.join(top, 'ffprobe-cwd'); const outputDir = path.join(scratch, 'out');
  for (const n of [outputDir, path.join(scratch, 'home'), path.join(scratch, 'tmp')]) fs.mkdirSync(n, {recursive: true});
  const options = postRenderProbeOptions(scratch);
  assert.equal(options.cwd, path.join(ROOT, BINARIES));
  assert.notEqual(options.cwd, ROOT);
  const profile = sandboxProfile({scratch, outputDir, readFiles: []});
  const result = spawnSync('/usr/bin/sandbox-exec', ['-p', profile, path.join(ROOT, BINARIES, 'ffprobe'), '-version'],
    {...options, encoding: 'utf8', timeout: 10000});
  assert.equal(result.status, 0, result.stderr || result.error?.message);
  assert.match(result.stdout, /ffprobe version/u);
});
