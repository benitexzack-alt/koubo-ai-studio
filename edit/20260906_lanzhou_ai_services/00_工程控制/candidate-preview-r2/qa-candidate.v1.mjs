...
const output=path.join(repo,base,'07_预览与质检/candidate-preview-r2');
...
const plan=JSON.parse(fs.readFileSync(path.join(repo,'remotion/src/lanzhou-services-v91-candidate-r2/visual-plan.v1.json')));
...
const checks={probeExit:probe.exitCode===0,decodeExit:decode.exitCode===0,signalScanExit:scan.exitCode===0,
  ...
  loudnessMeasured:Number.isFinite(loudness),truePeakMeasured:Number.isFinite(truePeak),truePeakHeadroom:Number.isFinite(truePeak)&&truePeak<=-1};
write('candidate-machine-qa.v1.json',{schemaVersion:'lanzhou-services-candidate-machine-qa/v1',
  ...
  sourceAudio:{hostGain:1,newsGain:.62,paperGain:.1,additionalSfxGain:'per-cue-measured',newsHostOverlap:false},
  attachedSfx:{count:plan.sfx.length,distinctFiles:new Set(plan.sfx.map(c=>c.publicPath)).size,cues:plan.sfx,userAudibilityConfirmed:false},
