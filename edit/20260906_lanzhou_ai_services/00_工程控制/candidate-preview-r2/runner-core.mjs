...
export const REVISION = '20260907-lanzhou-services-candidate-preview-r2';
export const CONTROL = `edit/${EPISODE}/00_工程控制/candidate-preview-r2`;
export const ENTRY = 'remotion/src/lanzhou-services-v91-candidate-r2/index.tsx';
export const PUBLIC = 'remotion/public/lanzhou-services-candidate-r2';
export const OUTPUT = `edit/${EPISODE}/07_预览与质检/candidate-preview-r2`;
...
export const COMPOSITION = 'LanzhouServicesV91CandidateR2';
export const MAX_STILLS = 64;
...
export const SHOTCRAFT_COPY = 'remotion/src/lanzhou-services-v91-candidate-r2/ShotcraftEffects.generated.tsx';
...
export function validatePermission(p, request, command) {
  ensure(p.schemaVersion === 'lanzhou-services-revision-preview-permission/v1', 'OLD_GRANT_FORBIDDEN');
  ...
  ensure(s.publicDir === PUBLIC && s.preserveOriginalFramesAndSubtitleTiming === true && s.preservePriorCandidate === true
    && digest(s.allowedAdjustments) === digest(['official-document-magnification-line-emphasis', 'top-left-type-size-hierarchy', 'sfx-audibility-opening-emphasis']), 'REVISION_SCOPE_INVALID');
  ensure(p.evidence?.kind === 'current-conversation-direct-user-confirmation' && p.evidence.userQuote?.trim()
    && p.evidence.requestedChangesSummary?.trim(), 'CURRENT_USER_EVIDENCE_REQUIRED');
...
export function validatePreservedInputs(permission, byPath, publicFiles) {
  ensure(Array.isArray(permission.preservedInputs) && permission.preservedInputs.length > 0
    && Array.isArray(permission.preservedPublicFiles) && permission.preservedPublicFiles.length === 13
    && Array.isArray(permission.approvedAdditionalPublicSfx), 'REVISION_BASELINE_REQUIRED');
  const expectedPublic = [...permission.preservedPublicFiles, ...permission.approvedAdditionalPublicSfx];
  ensure(new Set(expectedPublic.map(b => b.path)).size === expectedPublic.length, 'REVISION_PUBLIC_DUPLICATE');
  for (const item of permission.approvedAdditionalPublicSfx) ensure(item.path?.startsWith(`${PUBLIC}/sfx/`)
    && /\.(wav|mp3|m4a)$/u.test(item.path) && item.confirmationQuote?.trim(), 'NEW_SFX_CONFIRMATION_REQUIRED');
  for (const item of [...permission.preservedInputs, ...expectedPublic]) {
    relativePath(item.path);
    ensure(SHA.test(item.sha256) && byPath.get(item.path) === item.sha256, 'PRESERVED_INPUT_DRIFT', item.path);
  }
  ensure(digest([...publicFiles].sort()) === digest(expectedPublic.map(b => b.path).sort()), 'UNAPPROVED_PUBLIC_CHANGE');
}
...
export function validateRequest({root = ROOT, kb = KB, requestPath, command, runtime, ts, now = Date.now()}) {
  ...
  ensure(Array.isArray(request.stillFrames) && request.stillFrames.length > 0 && request.stillFrames.length <= MAX_STILLS
