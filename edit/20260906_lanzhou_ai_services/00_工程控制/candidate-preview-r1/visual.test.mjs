...
const plan = read('remotion/src/lanzhou-services-v91-candidate-r1/visual-plan.v1.json');
...
test('完整原始Shotcraft组件按字节复用',()=>{
  assert.equal(sha('skills/koubo-shotcraft-library/assets/ShotcraftEffects.tsx'),sha('remotion/src/lanzhou-services-v91-candidate-r1/ShotcraftEffects.generated.tsx'));
});
test('画面主层半开区间不重叠，音效每卡反向绑定',()=>{
  ...
  for(const row of rows)assert(plan.sfx.some(c=>c.visualId===row.id));
});
